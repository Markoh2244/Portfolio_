...
def getCell(app,x,y):
    row=(x-260)//app.cellSize
    col=(y-100)//app.cellSize
    return col,row
...
def mousePressed(app,event):
    ...
    if event.x<app.margin+app.cellSize and event.x>app.margin and event.y>400 and event.y<400 +app.cellSize:
        app.color='red'
    if event.x<(app.margin+2*app.cellSize) and event.x>app.margin +app.cellSize and app.y>400 and event.y<400+ app.cellSize:
        app.color='gray'
    
    if (event.x<(260 + (app.cellSize*app.rows))) and event.x>260 and event.y<(100+(app.cellSize*app.cols)) and event.y> 100:
...
def createBoard(app,canvas):
    for x in range(app.cols):
        for i in range(app.rows):
            canvas.create_rectangle((i*app.cellSize)+260,(x*app.cellSize)+100,(i*app.cellSize)+app.cellSize+260,(x*app.cellSize)+app.cellSize+100,fill=app.bboard[x][i][0])
            if (x,i) not in (app.locations) and app.bboard[x][i][1] != '0':
                canvas.create_text((i*app.cellSize)+260+ (app.cellSize//2), (x*app.cellSize)+100 + (app.cellSize//2), text=app.bboard[x][i][1],fill="black", font="Helvetica 12 bold")
    for x in range(app.cols):
        for i in range(app.rows):
            canvas.create_rectangle((i*app.cellSize)+260,(x*app.cellSize)+100,(i*app.cellSize)+app.cellSize+260,(x*app.cellSize)+app.cellSize+100,fill=app.board[x][i])
