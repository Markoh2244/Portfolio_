...
def getCell(app,x,y):
    row=(x-app.margin)//app.cellSize
    col=(y-app.margin)//app.cellSize
    return col,row
...
def mousePressed(app,event):
    ...
    if app.x<app.margin+app.cellSize and app.x>app.margin and app.y>400 and app.y<400 +app.cellSize:
        app.color='red'
    if app.x<(app.margin+2*app.cellSize) and app.x>app.margin +app.cellSize and app.y>400 and app.y<400+ app.cellSize:
        app.color='gray'
    
    if (event.x<(app.margin + (app.cellSize*app.rows))) and event.x>app.margin and event.y<(app.margin+(app.cellSize*app.cols)) and event.y> app.margin:
...
def createBoard(app,canvas):
    for x in range(app.cols):
        for i in range(app.rows):
            canvas.create_rectangle((i*app.cellSize)+app.margin,(x*app.cellSize)+app.margin,(i*app.cellSize)+app.cellSize+app.margin,(x*app.cellSize)+app.cellSize+app.margin,fill=app.bboard[x][i][0])
            if (x,i) not in (app.locations) and app.bboard[x][i][1] != '0':
                canvas.create_text((i*app.cellSize)+app.margin + (app.cellSize//2), (x*app.cellSize)+app.margin + (app.cellSize//2), text=app.bboard[x][i][1],fill="black", font="Helvetica 12 bold")
    for x in range(app.cols):
        for i in range(app.rows):
            canvas.create_rectangle((i*app.cellSize)+app.margin,(x*app.cellSize)+app.margin,(i*app.cellSize)+app.cellSize+app.margin,(x*app.cellSize)+app.cellSize+app.margin,fill=app.board[x][i])
    ...
    canvas.create_rectangle(590,10,app.margin+(app.rows*app.cellSize),40,fill='white')##########################################
    ...
    canvas.create_rectangle(app.margin+app.cellSize,app.bottom1,app.margin+(2*app.cellSize),app.bottom2,fill='red')
